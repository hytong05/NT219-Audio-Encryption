...
def generate_decryption_key(Length, ShareKey, InitialVector):
...
def decrypt_song(CipherVoice, ShareKey, InitialVector):
    ...
    IntermediateKey1, IntermediateKey2, SineCosineSequence, LogisticSineCosineSequence, I0, I1, I2, I3 = generate_decryption_key(Length, ShareKey, InitialVector)
...
def decryption(path, ShareKey, InitialVector, pathOutput):
  # Đọc file âm thanh
  CipherVoice, sample_rate = Input(path)
  
  for i in range(len(CipherVoice)):
    if CipherVoice[i] < 0:
  ...
  num_chunks = len(CipherVoice) // chunk_size
  
  # Chuẩn bị list để lưu bản rõ của từng chunk
  chunk_plain_list = []
  ...
  for i in range(num_chunks):
    ...
    chunk = CipherVoice[start_time:end_time]
            
    # Giải mã từng chunk
    chunk_decrypt = decrypt_song(chunk, ShareKey, InitialVector)
    
    # Lưu chunk được giải mã vào danh sách
    chunk_plain_list.append(chunk_decrypt)
    
  
  if num_chunks * chunk_size != len(CipherVoice):
    chunk = CipherVoice[end_time:len(CipherVoice)]
  ...
  chunk_plain = np.concatenate(chunk_plain_list)
  
  Output(pathOutput, sample_rate, chunk_plain)
